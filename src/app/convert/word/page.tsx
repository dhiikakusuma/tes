...
import { FileType2, Loader2, Download, FileText } from "lucide-react";
...
export default function ConvertWordPage() {
    ...
    try {
      ...
      pages.forEach((pg, idx) => {
        ...
        if (pg.lines.length === 0) {
          children.push(
            new Paragraph({
              children: [new TextRun({ text: "(no extractable text)", italics: true })],
            }),
          );
        } else {
      ...
      downloadBlob(blob, `${stripExt(file.name)}.docx`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <ToolPageHeader
        title="PDF → Word"
        description="Extract text from each page into an editable .docx document."
        icon={FileType2}
        ...
        <Card>
          ...
          <CardBody className="space-y-3">
            <p className="text-sm text-zinc-600 dark:text-zinc-400">
              The conversion extracts plain text from each page. Images,
              complex layouts, and tables are not preserved.
            </p>
            {busy && (
              <div>
